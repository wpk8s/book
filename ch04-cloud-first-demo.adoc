...
Within the list, if there is also the linux kernel (starts with linux-image),
please do a reboot of the virtual server after it finishes and wait around 30
seconds before you ssh back in.
...
It will download and install the latest version of it, and we can repeat
our previous WordPress deployment.

TIP: Visual Studio Code users. Install the *https://j.mp/2MwT3ZO[Remote SSH]*
extension, and follow this https://j.mp/3tzUGGs[help]
for making life easy when you need to work with a lot of cloud
servers. I will use it as an example from now on.
...
Create a folder named `wordpress` and within it, use *nano* or your favorite
editor to create the same files we did previously.
...
server and now it allows you to edit files like you would locally and
if you open a Visual Studio Code *Terminal* it is actually opening the
prompt through ssh in the cloud server. Just enjoy it's simplicity.
...
for the first time deployment, let's put the ip of the cloud server 
in our local hosts file to test our online WordPress installation.
...
NOTE: I did not set anything up for *HTTPS* yet as we are going to look
at different ways we can set that up, as you might need to know each one,
depending on how your clients need to have *HTTPS* configured.

WARNING: Do not leave this demo WordPress available online as it is.
Run it's deletion command again `sudo microk8s kubectl delete -k ./` and
you could even delete the cloud server (you will need to recreate it for
future chapters).

OK, so we did it, we had live online WordPress, running on MicroK8s.
What is next?

In the following chapter we will concentrate on how to prepare _recipes_
to ensure the services use the exact versions we need and we will experiment
in upgrading them, as this is a critical requirement in a secured
environment and also a necessity for avoiding bugs.

Also, we will see how to setup multiple websites. We will explore running
WordPress with auto updates and then look into locking and managing
WordPress like any other modern php project, by using
https://j.mp/3rt7ay4[Composer] and https://j.mp/2LtvPmK[Roots - Bedrock]
for modern WordPress development, that can help us to put WordPress in
...
ElasticSearch to have blazing speed in searches, and instant suggestions,
and add caching using Redis, add monitoring and observability, alerts
and many other things. If I have really captured your attention, I do hope you
